...
/**
 * 迟滞切档判定：当前档位为低分辨率时超过升档阈值才换原图；当前为原图时降到降档阈值以下才回低分辨率。
...
/**
 * 是否需要为某个素材生成低分辨率版本。GIF（动图重编码会丢动画）与本身就足够小的图跳过。
