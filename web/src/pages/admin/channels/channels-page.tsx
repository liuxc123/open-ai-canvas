...
type ChannelFormValues = { name: string; baseUrl: string; materialBaseUrl?: string; materialApiVersion?: string; apiKey?: string; secretKey?: string; headers?: ChannelHeader[]; useGlobalConcurrency?: boolean; concurrencyLimit?: number; enabled?: boolean };
...
export default function ChannelsPage() {
    ...
    const openDrawer = (channel?: ModelChannel) => {
        ...
        form.resetFields();
        form.setFieldsValue(channel ? { name: channel.name, baseUrl: channel.baseUrl, materialBaseUrl: channel.materialBaseUrl || "", materialApiVersion: channel.materialApiVersion || "", apiKey: "", secretKey: "", headers: channel.headers || [], useGlobalConcurrency: !channel.concurrencyLimit, concurrencyLimit: channel.concurrencyLimit || undefined, enabled: channel.enabled !== false } : { name: "", baseUrl: "", materialBaseUrl: "", materialApiVersion: "", apiKey: "", secretKey: "", headers: [], useGlobalConcurrency: true, concurrencyLimit: undefined, enabled: true });
        setDrawerOpen(true);
    };
    ...
    const save = async () => {
        ...
        setSaving(true);
        try {
            const payload = { name: values.name.trim(), baseUrl: values.baseUrl.trim(), materialBaseUrl: values.materialBaseUrl?.trim() || "", materialApiVersion: values.materialApiVersion?.trim() || "", apiKey: values.apiKey?.trim() || "", secretKey: values.secretKey?.trim() || "", headers: values.headers || [], useGlobalConcurrency: values.useGlobalConcurrency !== false, concurrencyLimit: values.useGlobalConcurrency === false ? values.concurrencyLimit : undefined, enabled: values.enabled !== false };
        ...
        <AdminPageFrame title="系统渠道" description="渠道、模型与售价" actions={<Button type="primary" icon={<Plus className="size-4" />} onClick={() => openDrawer()}>新增系统渠道</Button>}>
            ...
            <Drawer title={editingChannel ? "编辑系统渠道" : "新增系统渠道"} open={drawerOpen} size="min(720px, 100vw)" onClose={closeDrawer} maskClosable={!saving} destroyOnHidden extra={<Button type="primary" loading={saving} onClick={() => void save()}>保存</Button>}>
                <Form form={form} layout="vertical" requiredMark={false}>
                    ...
                    <Form.Item name="materialApiVersion" label="资产 API 协议版本（可选）" extra="默认 v1；修改后会强制所有素材重新注册。"><Input placeholder="v1" /></Form.Item>
