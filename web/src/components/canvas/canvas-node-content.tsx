...
function ImageNodeContent(props: CanvasNodeContentProps) {
    ...
    return (
        <ImageContent
            ...
            theme={props.theme}
            scale={props.scale}
            reduceMediaEffects={props.reduceMediaEffects}
...
function ImageContent({
    ...
    batchRecovering,
    onToggleBatch,
}: Pick<CanvasNodeContentProps, "node" | "theme" | "scale" | "reduceMediaEffects" | "isBatchRoot" | "batchCount" | "batchExpanded" | "batchOpening" | "batchRecovering" | "onToggleBatch">) {
...
function DeferredMediaLoad({ icon, label, disabled, onClick }: { icon: ReactNode; label: string; disabled: boolean; onClick: () => void }) {
    return (
        <button
            type="button"
            data-canvas-no-zoom
            className="flex size-full flex-col items-center justify-center gap-2 rounded-[var(--node-radius)] bg-black text-white/75 transition-colors hover:text-white disabled:cursor-wait"
            disabled={disabled}
            onClick={(event) => {
                event.stopPropagation();
                onClick();
...
const OFFSCREEN_UNLOAD_DELAY_MS = 3000;

/**
 * 双向视口观察：进入边缘 600px 内视为"靠近"（触发加载），离开超过 600px 且持续 3 秒才视为"远离"
 * （触发卸载）。延迟卸载避免了缓慢平移时节点在画布边缘反复加载/卸载。
 */
function useNearViewport(ref: RefObject<Element | null>) {
    ...
    useEffect(() => {
        ...
        const observer = new IntersectionObserver(
            ...
            { rootMargin: `${NEAR_VIEWPORT_ROOT_MARGIN_PX}px` },
        );
