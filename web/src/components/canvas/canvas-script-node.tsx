...
import { ChevronDown, ChevronUp, Clapperboard, Copy, Download, Expand, Film, Grid3X3, Image as ImageIcon, ListTree, Merge, Minus, MoreHorizontal, Plus, RefreshCw, Send, Square, Trash2, Upload, Video, X } from "lucide-react";
...
export function CanvasScriptNodeContent({
    ...
    const moreMenuItems: MenuProps["items"] = [
        ...
        { key: "export-excel", icon: <Download className="size-3.5" />, label: "导出 Excel", disabled: !rows.length, onClick: () => onExportExcel() },
        { type: "divider" },
    ...
    return (
        <Modal title={node?.title || "分镜脚本"} open={open} onCancel={onClose} footer={null} width="min(1480px, calc(100vw - 40px))" centered destroyOnHidden>
            <div className="mb-3 flex flex-wrap items-center gap-2">
                ...
                {onImportExcel ? (
                    <Button icon={<Upload className="size-4" />} onClick={onImportExcel}>
                        导入 Excel
                    </Button>
                ) : null}
                {onExportExcel ? (
                    <Button icon={<Download className="size-4" />} disabled={!rows.length} onClick={onExportExcel}>
                        导出 Excel
                    </Button>
                ) : null}
